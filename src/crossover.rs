...
pub fn simple_sub_path_crossover(parent1: &Genotype, parent2: &Genotype) -> Genotype {
    let mut rng = rand::thread_rng();

    let max = parent1.stops.len();
    let point_1: usize = rng.gen_range(0..(max-5));
    let point_2: usize = point_1 + rng.gen_range(0..5);
    ...
    let search_itr = child.stops.iter();
    for v in p1_slice{
        let idx = &child.stops.iter().position(|n_stop| v == n_stop ).unwrap();
        child.stops.remove(*idx);
    }

    let drop_at: usize = rng.gen_range(0..(child.stops.len()));
    let mut snip = child.stops.split_off(drop_at);
    child.stops.extend_from_slice(p1_slice);
    child.stops.append(&mut snip);
